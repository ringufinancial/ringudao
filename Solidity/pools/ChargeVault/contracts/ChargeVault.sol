 ...
 */
contract ChargeVault is ERC20, Ownable, ReentrancyGuard, IVault {
	...
	function balance() public view returns (uint256) {
		return
			want().balanceOf(address(this)).add(
				IVaultStrategy(strategy).balanceOf()
			);
	}
	...
	function proposeStrat(address _implementation) public onlyOwner {
		require(
			address(this) == IVaultStrategy(_implementation).vault(),
	...
	function upgradeStrat() public onlyOwner {
		...
		strategy = IVaultStrategy(stratCandidate.implementation);
		stratCandidate.implementation = address(0);
